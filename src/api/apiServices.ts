...
export const getRatedMovies = async (token: string, page = 1) => {
  const response = await axios.get(
    `https://api.themoviedb.org/3/guest_session/${token}/rated/movies?api_key=${apiKey}&language=en-US&sort_by=created_at.asc&page=${page}`
  );
  return response;
};
...
export const setrate = (id: string, rate: string) => localStorage.setItem(id, rate);
export const getRate = (id: string) => localStorage.getItem(id);
